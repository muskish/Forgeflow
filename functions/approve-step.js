const { executeAdminGraphQL, verifyUserRole, callLLM } = require('./_shared');
...
async function approveStepHandler(req, res) {
  try {
    ...
    const remainingSteps = allSteps.filter((s) => s.step_order > currentStepOrder);
    ...
    let previousOutput = priorData.step_runs?.[0]?.output || null;
...
module.exports = approveStepHandler;
